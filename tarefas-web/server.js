...
const app = express();
const PORT = 3002; // Porta padrão para desenvolvimento

// const PORT = 3002; // Porta alternativa para evitar conflitos
...
app.get('/api/tasks', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Erro ao carregar tarefas', details: error.message });
    }
});
...
app.post('/api/tasks', async (req, res) => {
    try {
        ...
        console.log('Recebendo dados para salvar:', tasks.length, 'tarefas');
        ...
        res.status(500).json({ error: 'Erro ao salvar tarefas', details: error.message });
    }
});
...
async function startServer() {
    await initDataFile();
    app.listen(PORT, () => {
        console.log(`Servidor da lista de tarefas rodando em http://localhost:${PORT}`);
